...
static NSString* isAllMagnetDownload;
@interface GiFHUD ()
...
+ (instancetype)instance {
    ...
    [instance setCenter:APPDELEGATE.window.center];
    return instance;
}
...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        [self.layer setBackgroundColor:(__bridge CGColorRef _Nullable)([UIColor clearColor])];//[[UIColor colorWithWhite:1 alpha:1.0] CGColor]];
        [self.layer setCornerRadius:10];
        [self.layer setMasksToBounds:YES];
        
         self.imageView = [[UIImageView alloc] initWithFrame:CGRectMake((Size/2)-75,(Size/2)-105,150, 150)];
         _lbl = [[UILabel alloc] initWithFrame:CGRectMake(100,200, 280, 50)];
         _lbl.numberOfLines = 11;
      //  _lbl.backgroundColor = [UIColor yellowColor];
         _lbl.textColor = [UIColor whiteColor];
        
        _lbl.text = @"Please wait..";

        _lbl.textAlignment = NSTextAlignmentJustified;

         NSRange redTextRange = [_lbl.text rangeOfString:@"Please wait.."];
        NSMutableAttributedString * attributedString= [[NSMutableAttributedString alloc] initWithString:_lbl.text];
        [attributedString addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:redTextRange];
        _lbl.attributedText = attributedString;
